...
const UpdatePaymentIn = () => {
    ...
    useEffect(() => {
        // Try to load from localStorage additions first, then from history state or defaults
        try {
        ...
        // If no saved item, try to read from location state item
        if (location?.state?.item) {
            const item = location.state.item;
            setPaymentData({
                ...item,
                status: determineStatus(item.totalAmount, item.payments || [])
            });
        }
    }, [paymentId, location]);
    ...
    const determineStatus = (total, payments) => {
        ...
        if (paid >= t && t > 0) return 'Full Received';
        if (paid > 0 && paid < t) return 'Partially Received';
        return 'Due';
    };
    ...
    const handleUpdate = () => {
        // persist to localStorage additions if present; otherwise just navigate back
        try {
            const saved = JSON.parse(localStorage.getItem('payment_in_additions') || '[]');
            ...
            console.error('Failed to persist payment in update', err);
        }
    ...
    return (
        <div className="update-payment-container">
            <div className="page-header">
                <div className="table-title">Update Payment In</div>
            </div>
                ...
                <table className="expense-input-table">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <span className={`status-badge ${paymentData.status === 'Full Received' ? 'badge-paid' : paymentData.status === 'Partially Received' ? 'badge-partial' : 'badge-unpaid'}`}>
                                    {paymentData.status}
                                </span>
                            </td>
                        </tr>
            ...
            <div className="form-actions">
                ...
                <button className="cancel-form-btn" onClick={() => navigate('/payment_in')}>Cancel</button>
            </div>
...
export default UpdatePaymentIn;
